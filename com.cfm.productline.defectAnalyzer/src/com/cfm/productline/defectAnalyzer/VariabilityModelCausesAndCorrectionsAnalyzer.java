...
import com.cfm.productline.util.PowerSetUtil;
...
public class VariabilityModelCausesAndCorrectionsAnalyzer extends
	 ...
	 */
	private Diagnostic identifyMCSandMUSes(
			CorrectionSetIdentificationType correctionSetIdentifcationType,
		...
		try {
				...
				if (combinatorialPowerSetByLevel.isEmpty()) {
					...
					if (!allMCSes.isEmpty()) {
						...
						if (correctionSetIdentifcationType
								.equals(CorrectionSetIdentificationType.PARTIAL)) {
							continueMCSesIdentification = Boolean.FALSE;
						}
					}

					// Si el editor es GNU prolog se ciera la conexión luego de
					// la
					// identificación delos MCS de cada nivel para evitar
					// excepción
			...
			Diagnostic diagnostic = createDiagnostic(allMCSes, defectToAnalyze);
			if (r > 2) {
			...
			/*if (correctionSetIdentifcationType
				...
				allMUSes = unsatisfiableCollectionOfSets;
					...
					.equals(CorrectionSetIdentificationType.COMPLETE)) {
				boolean timeOverCauses = HittingSetIdentifier.filterMUSesTime(
						allMCSes, new ArrayList<Dependency>(), allMUSes,
						new Long(60000));
				diagnostic.setTimeOverCauses(timeOverCauses);
			}*/
		...
		while (continueMCSIdentification) {
			...
			if (dependendencyToRemoveList != null) {
				...
				if (isSatisfiable) {
					...
					// Si el editor es GNU Prolog se cierra la conexión y se
					// vuelve
					// a abrir para evitar
					// excepciones de tipo
					// Atom table full (max atom: 32768, environment
					// variable used: MAX_ATOM)
					if (countAnalyzedInstancesByR == GNU_PROLOG_MAX_ATOM
							&& prologEditorType
									.equals(SolverEditorType.GNU_PROLOG)) {
						countAnalyzedInstancesByR = 0;
						SolverOperationsUtil.stopProlog();
						SolverOperationsUtil.starPrologEngine();
						System.out
								.println("!!!!!!!!!!!!! ME CERRÉ !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! :(");
	...
	public Diagnostic analyzeCausesCorrectionsRedundancies(
			Defect defectToAnalyze,
			Map<Long, Dependency> modelDependenciesList,
			Map<Long, Dependency> fixedDependenciesList,
			CorrectionSetIdentificationType correctionSetIdentifcationType) throws FunctionalException {
	...
	public Diagnostic analyzeCausesOneDefect(Defect defectToAnalyze,
		...
		if (correctionSetIdentifcationType != null) {
			Diagnostic diagnostic = identifyMCSandMUSes(
					correctionSetIdentifcationType, modelDependenciesList,
	...
	public VMCauseAnalyzerOutDTO causesAnalyzer(
		...
		// Map<Defect, Diagnostic> defectDiagnosticMap = new HashMap<Defect,
		// Diagnostic>();
		...
		if (vmCauseAnalizerInDTO.getVoidModel() != null
			...
			Diagnostic diagnosticVoid = invokeCausesIdentiferOneDefect(
					voidModelDefect,
					vmCauseAnalizerInDTO.getCorrectionSetIdentifcationType());
		...
		if ((identifyCausesDeadFeatures || identifyCausesFalseProductLine)
			...
			for (Defect deadFeature : vmCauseAnalizerInDTO
				...
				// mapa
				// if
				// (deadFeature.getId().equals("Minutos")||deadFeature.getId().equals("Segundos")
				// ) {
				Diagnostic diagnosticDeadFeatures = invokeCausesIdentiferOneDefect(
						deadFeature,
						vmCauseAnalizerInDTO
								.getCorrectionSetIdentifcationType());
		...
		if ((identifyCausesFalseOptional || identifyCausesFalseProductLine)
			...
			for (Defect falseOptionalFeature : vmCauseAnalizerInDTO
				...
				// if (falseOptionalFeature.getId().equals("Registro")) {

				Diagnostic diagnosticFalseOptional = invokeCausesIdentiferOneDefect(
		...
		if (identifyCausesRedundancies
			...
			for (Defect redundancy : vmCauseAnalizerInDTO.getRedundancies()) {
				Diagnostic diagnosticRedundancy = analyzeCausesCorrectionsRedundancies(
	...
	public void printResults(Diagnostic diagnostic) {
		// 3. PRINT RESULTS
	...
	private Diagnostic invokeCausesIdentiferOneDefect(Defect defect,
			CorrectionSetIdentificationType correctionSetIdentifcationType) throws FunctionalException {
		...
		if (defect instanceof Redundancy) {
			diagnostic = analyzeCausesCorrectionsRedundancies(defect,
					...
					correctionSetIdentifcationType);
