...
public class SXFMWriter {
	public void writeSXFM(ProductLine am, File outFile) throws IOException {
	...
	public String getSXFMContent(ProductLine am) {
	...
	private void printConstraint(Constraint c, StringBuffer buf) {
		...
		if (c instanceof GenericConstraint) {
			fillBlank(buf, 1);
			buf.append(c.getIdentifier()).append(": ")
					.append(((GenericConstraint) c).getText()).append("\n");
		}
	}
	...
	private void printFeatureTree(ProductLine pl, StringBuffer str) {
		...
		VariabilityElement root = pl
				.getVariabilityElement((String) rootsArray[0]);
	...
	private void printFeatureBranch(ProductLine pl, StringBuffer str,
			String type, VariabilityElement varPoint, int depth,
			Set<VariabilityElement> visited) {
		...
		for (Constraint c : pl.getConstraintsFor(varPoint)) {
			if (c instanceof OptionalConstraint) {
				...
				if (opC.getFeature1Id().equals(varPoint.getIdentifier())) {
					VariabilityElement p = pl.getVariabilityElement(opC
							.getFeature2Id());
			...
			if (c instanceof GroupConstraint) {
				...
				if (gc.getParent().equals(varPoint.getIdentifier())) {
					...
					for (int i = 1; i < related.size(); i++) {
						VariabilityElement p = pl.getVariabilityElement(related
								.get(i));
						// printFeatureBranch(pl, str, vp, depth + 1, visited);
	...
	// private void printOptionalLine(VariabilityElement p, StringBuffer str,
	// int depth){
	// fillBlank(str, depth);
	// str.append(":o ").append(p.getPropertyAs(Property.NAME)).append(" (").append(p.getIdentifier()).append(")\n");
	// }

	private void printNodeLine(VariabilityElement p, String type,
			StringBuffer str, int depth) {
		str.append(":").append(type);
		// if( depth == 0 )
		// str.append("r");
		str.append(" ").append(p.getName()).append(" (")
				.append(p.getIdentifier()).append(")\n");
	}

	private void printGroupLine(GroupConstraint gc, StringBuffer str, int depth) {
		String upper = (gc.getUpperLimit() == GroupConstraint.INFINITE) ? "*"
				: gc.getUpperLimit() + "";
		fillBlank(str, depth);
		str.append(":g [").append(gc.getLowerLimit()).append(",").append(upper)
				.append("]\n");
	}

	// private void printMandatoryLine(VariabilityElement p, StringBuffer str,
	// int depth) {
