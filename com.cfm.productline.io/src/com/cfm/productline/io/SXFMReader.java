		...
		if( formula.countVars() == 2 ){
			...
			if( !literals.get(0).isPositive() ){
				if( !literals.get(1).isPositive())
					//~A or ~B
					c = new ExcludesConstraint(literals.get(0).getVariable().getID(), literals.get(1).getVariable().getID());
				else
					c = new RequiresConstraint(literals.get(0).getVariable().getID(), literals.get(1).getVariable().getID());
			}else{
