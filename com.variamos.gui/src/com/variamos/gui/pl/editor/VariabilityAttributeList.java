...
public class VariabilityAttributeList extends JList<Variable> {
	...
	private Variable spoof = new Variable("Add ...", "Add ...",
	...
	protected void editItem(Variable var) {
		...
		// Name
		final Variable name = new Variable("Name", var.getName(),
				StringType.IDENTIFIER);
		...
		// Domain
		final Variable domain = new Variable("Domain", domainRepresentation,
				StringType.IDENTIFIER);

		final VariabilityParameterDialog dialog = new VariabilityParameterDialog(
				editor, name, domain);
		dialog.setOnAccept(new DialogButtonAction() {
			...
			public boolean onAction() {
				...
				// FIXME verify if 0 is always OK
				v.setDomain(DomainParser.parseDomain(
						(String) domain.getValue(), 0));

				// System.out.println( v.getDomain() );

				if (insert) {
					((DefaultListModel<Variable>) getModel()).insertElementAt(
							v, getModel().getSize() - 1);
