 ...
 * class [VariamosGUIEditorActions]
 * {@link com.variamos.gui.core.viewcontrollers.VariamosGUIPerpectiveEditorActions}
 * 
...
public class VariamosGUIPerspectiveEditorActionsController {
	...
	public static void newAction(VariamosGraphEditor variamosEditor) {
		if (variamosEditor != null) {
			...
			// When the perspective is the one of configuration, then the action
			// for new is
			// not supported.
			...
			// When there are not changes or the user accepts to loose changes,
			// then the
			// editor is reseted
			if (!variamosEditor.isModified()
					|| JOptionPane.showConfirmDialog(variamosEditor,
							mxResources.get("loseChanges")) == JOptionPane.YES_OPTION) {
				variamosEditor.resetView();
				System.runFinalization();
				if (variamosEditor.getPerspective() == 2) {
					((MainFrame) variamosEditor.getFrame()).setPerspective(2);
					((MainFrame) variamosEditor.getFrame()).setLayout();
					VariamosGUIPerspectiveEditorActionsController
							.changeVariamosParadigmView(
									((MainFrame) variamosEditor.getFrame()),
									((MainFrame) variamosEditor.getFrame())
											.getGraphEditors(),
									((MainFrame) variamosEditor.getFrame())
											.getFilesUrl(),
									((MainFrame) variamosEditor.getFrame())
											.getGraphEditors().get(1));

				}
				if (variamosEditor.getPerspective() == 1) {

					// Call the action that defines the default elements in the
					// semantic model. True
					// means the model is empty
					InstanceModel instanceModel = variamosEditor
							.getEditedModel();
					APIDynsup.createOperationsSuperstructure(instanceModel,
							Boolean.TRUE);

					// The instance model under edition is updated into the
					// editor
					((PerspEditorGraph) ((VariamosGraphComponent) variamosEditor
	...
	public static void changeVariamosParadigmView(final MainFrame mainFrame,
			List<VariamosGraphEditor> graphEditors, String filesUrl,
			VariamosGraphEditor activeGraphEditor) {
		ParadigmTypeEnum paradigmChoosed = ParadigmChooserPane
				.showInputDialog();
		/*
		 * if(paradigmChoosed==null ||
		 * paradigmChoosed.equals(ParadigmTypeEnum.EMPTY)) { dispose();
		 * System.exit(0); }
		 */
		...
		case CONSTRAINTGRAPHS:
			...
			break;
		case CUSTOMIZED:
			semantic = VariamosGUIPerspectiveEditorActionsController
					.getFileFromChooser(graphEditors.get(0));
			syntax = VariamosGUIPerspectiveEditorActionsController
					.getFileFromChooser(graphEditors.get(2));
			break;
		case EMPTY:
			break;
		case FEATURES:
		...
		if (syntax != null && semantic != null) {
			System.out.println("syntax:" + syntax.exists() + ", semantic:"
					+ semantic.exists());
			final VariamosGraphEditor graphEditor = activeGraphEditor;
			final VariamosGraphEditor syntaxGraphEditor = graphEditors.get(2);
			final File sintaxFile = syntax;
			VariamosGUIPerspectiveEditorActionsController.loadAction(
					graphEditors.get(0), semantic, new Runnable() {
						@Override
						public void run() {
							VariamosGUIPerspectiveEditorActionsController
									.loadAction(syntaxGraphEditor, sintaxFile,
											new Runnable() {
												@Override
												public void run() {
													graphEditor
															.updatePespectiveMenuTab(
																	mainFrame,
																	graphEditor,
																	mxResources
																			.get("syntaxPerspButton"));
													graphEditor
															.updatePespectiveMenuTab(
																	mainFrame,
																	syntaxGraphEditor,
																	mxResources
																			.get("modelingPerspButton"));
	 ...
	 *            saveAction
	 */
	private static void callLoadSaveFileTask(FileTasksEnum execType, File file,
			VariamosGraphEditor variamosEditor, mxGraph graph, String message,
			String ext, Color bgColor) {
		VariamosGUIPerspectiveEditorActionsController.callLoadSaveFileTask(
				execType, file, variamosEditor, graph, message, ext, bgColor,
				null);
	}

	private static void callLoadSaveFileTask(FileTasksEnum execType, File file,
			VariamosGraphEditor variamosEditor, mxGraph graph, String message,
			String ext, Color bgColor, final Runnable callBack) {
		...
		// Create the fileTask which is a swiworker that notifies progress in
		// background
		FileTasks task = null;
		switch (execType) {
		case OPEN_MODEL:
			task = new FileTasks(progressMonitor, execType, file,
					variamosEditor, graph, callBack);
			break;
		case SAVE_MODEL:
		case SAVE_IMAGE_SVG:
			progressMonitor.setMillisToDecideToPopup(0);
			progressMonitor.setMillisToPopup(0);
			task = new FileTasks(progressMonitor, execType,
					file.getAbsolutePath(), ext, variamosEditor, graph);
		case SAVE_IMAGE_PNG:
		case SAVE_IMAGE_OTHERS:
			progressMonitor.setMillisToDecideToPopup(0);
			progressMonitor.setMillisToPopup(0);
			task = new FileTasks(progressMonitor, execType,
					file.getAbsolutePath(), ext, variamosEditor, graph, bgColor);
			break;
		}
	...
	private static boolean loadActionBefore(VariamosGraphEditor variamosEditor) {
		if (variamosEditor != null) {
			...
			if (!variamosEditor.isModified()
				...
				return VariamosGUIPerspectiveEditorActionsController
						.graphExists(variamosEditor);
			}
		}
		return false;
	}
	...
	public static void loadAction(VariamosGraphEditor variamosEditor) {
		...
		if (VariamosGUIPerspectiveEditorActionsController
				.loadActionBefore(variamosEditor)) {
			File f = VariamosGUIPerspectiveEditorActionsController
					.getFileFromChooser(variamosEditor);
			if (f != null) {
				VariamosGUIPerspectiveEditorActionsController.loadAction(
						variamosEditor, f, null);
			}
			VariamosGUIPerspectiveEditorActionsController
					.loadActionAfter(variamosEditor);
		}
	}
	...
	private static File getFileFromChooser(VariamosGraphEditor variamosEditor) {
		...
		String path = (variamosEditor.getLastDir() != null) ? variamosEditor
				.getLastDir() : System.getProperty("user.dir");
		...
		DefaultFileFilter defaultFilter = new DefaultFileFilter("."
				+ fileExtension, fileExtensionName + " (." + fileExtension
				+ ")") {
			@Override
		...
		int fileChooserAnswer = fileChooser.showDialog(null,
	 ...
	 */
	public static void loadAction(VariamosGraphEditor variamosEditor,
			File theFile, final Runnable callBack) {
		if (VariamosGUIPerspectiveEditorActionsController
				.graphExists(variamosEditor)) {
			mxGraph graph = variamosEditor.getGraphComponent().getGraph();
			...
			VariamosGUIPerspectiveEditorActionsController.callLoadSaveFileTask(
					FileTasksEnum.OPEN_MODEL, theFile, variamosEditor, graph,
	 ...
	 */
	public static void saveAction(VariamosGraphEditor variamosEditor,
			boolean showDialog) {
		...
		if (variamosEditor != null) {
			...
			mxGraphComponent graphComponent = variamosEditor
					.getGraphComponent();
			...
			if (showDialog || variamosEditor.getCurrentFile() == null) {
				...
				filename = FileUtils.getFileNameByFileChooserToSaveFiles(
						variamosEditor, defaultExtension);

				// The name file already exist but the user don't want override
				// the file, then
				// nothing is done
			...
			if (ext.equalsIgnoreCase("svg")) {
				...
				VariamosGUIPerspectiveEditorActionsController
						.callLoadSaveFileTask(FileTasksEnum.SAVE_IMAGE_SVG,
								currentFile, variamosEditor, graph,
								"File saving", ext, null);

			} else if (ext.equalsIgnoreCase(defaultExtension)
					|| ext.equalsIgnoreCase("xml")) {
				// Call the swi worker for saving files
				File currentFile = new File(filename);
				VariamosGUIPerspectiveEditorActionsController
						.callLoadSaveFileTask(FileTasksEnum.SAVE_MODEL,
								currentFile, variamosEditor, graph,
								"File saving", ext, null);
			} else {
				Color bg = null;
				...
				if ((variamosEditor.getCurrentFile() != null && ext
						.equalsIgnoreCase("png"))) {
					// Call a SWING worker for saving images in png format
					VariamosGUIPerspectiveEditorActionsController
							.callLoadSaveFileTask(FileTasksEnum.SAVE_IMAGE_PNG,
									currentFile, variamosEditor, graph,
									"File saving", ext, bg);
				} else {
					// Call a SWING worker for saving images in other formats
					VariamosGUIPerspectiveEditorActionsController
							.callLoadSaveFileTask(
									FileTasksEnum.SAVE_IMAGE_OTHERS,
									currentFile, variamosEditor, graph,
									"File saving", ext, bg);
				}
			}
